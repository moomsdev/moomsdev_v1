...
abstract class AbstractPostType
{
    ...
    public function __construct()
    {
        ...
        add_action('carbon_fields_register_fields', function () {
            Container::make('post_meta', __('Cài đặt', 'mooms'))
                     ...
                     ->add_fields([
                         Field::make('checkbox', 'is_feature', __('Nổi bật', 'mooms'))->set_default_value(false),
                     ]);
        });
    ...
    protected function getLabels()
    {
        return [
            ...
            'add_new'               => __('Thêm mới', 'mooms'),
            'add_new_item'          => __('Thêm mới ' . $this->singularName, 'mooms'),
            'edit_item'             => __('Sửa', 'mooms'),
            'new_item'              => __('Mục mới', 'mooms'),
            'view_item'             => __('Chi tiết', 'mooms'),
            'search_items'          => __('Tìm kiếm', 'mooms'),
            'not_found'             => __('Không tìm thấy', 'mooms'),
            'not_found_in_trash'    => __('Không tìm thấy mục nào bị xóa', 'mooms'),
            'parent_item_colon'     => __('Mục cha:', 'mooms'),
            'all_items'             => __('Tất cả các mục', 'mooms'),
            'archives'              => __('Các mục được lưu trữ', 'mooms'),
            'insert_into_item'      => __('Chèn vào mục này', 'mooms'),
            'uploaded_to_this_item' => __('Đính kèm vào mục này', 'mooms'),
            'featured_image'        => __('Hình đại diện', 'mooms'),
            'set_featured_image'    => __('Chọn hình đại diện này', 'mooms'),
            'remove_featured_image' => __('Xóa hình đại diện', 'mooms'),
            'use_featured_image'    => __('Sử dụng làm hình đại diện', 'mooms'),
...
<?php get_footer() ?>');
    ...
    public function editAdminColumn($columns): array
    {
        ...
        $columns     = insertArrayAtPosition($columns, ['is_feature' => __('Nổi bật', 'mooms')], $totalColumn - 1);
    ...
    public function addMetaBoxBanDo()
    {
        add_action('add_meta_boxes', function () {
            add_meta_box('mooms_meta_map_box', __('Vị trí trên bản đồ', 'mooms'), [$this, 'hienThiMetaBoxBanDo'], $this->post_type, 'normal', 'high');
        });
    ...
    public function hienThiMetaBoxBanDo($post)
    {
        ...
        wp_enqueue_script('mooms-map-libraries', 'https://maps.googleapis.com/maps/api/js?key=' . $apiKey . '&libraries=geometry,places,drawing', [], '1.0', true);
        wp_enqueue_script('mooms-map-control', adminAsset('js/mooms-meta-box-ban-do.js'), [], '1.0', true);
    ...
    {
        Container::make('post_meta', __('Thông tin vị trí điểm đầu - cuối', 'mooms'))
                 ->set_context('carbon_fields_after_title')// normal, advanced, side or carbon_fields_after_title
                 ->set_priority('default')// high, core, default or low
                 ->where('post_type', 'IN', [$this->post_type])
                 ->add_fields([
                     Field::make('text', 'dia_chi_dau', __('Địa chỉ (điểm đầu)', 'mooms'))
                          ->set_width(100),
                     Field::make('select', 'tinh_dau', __('Tỉnh / Thành phố (điểm đầu)', 'mooms'))
                          ...
                          ->set_width(33),
                     Field::make('select', 'quan_huyen_dau', __('Quận / Huyện (điểm đầu)', 'mooms'))
                          ->add_options(['---- Chọn quận / huyện ----'])
                          ->set_width(33),
                     Field::make('select', 'phuong_xa_dau', __('Phường / Xã (điểm đầu)', 'mooms'))
                          ->add_options(['---- Chọn phường / xã ----'])
                          ->set_width(34),
                     Field::make('text', 'toa_do_x_dau', __('Tọa độ X (điểm đầu)', 'mooms'))
                          ->set_width(50),
                     Field::make('text', 'toa_do_y_dau', __('Tọa độ Y (điểm đầu)', 'mooms'))
                          ->set_width(50),

                     Field::make('text', 'dia_chi_cuoi', __('Địa chỉ (điểm cuối)', 'mooms'))
                          ->set_width(100),
                     Field::make('select', 'tinh_cuoi', __('Tỉnh / Thành phố (điểm cuối)', 'mooms'))
                          ->add_options(function () {
                              $posts   = get_posts([
                                  'post_type'      => 'dia-phan-tinh',
                                  'posts_per_page' => -1,
                          ...
                          ->set_width(33),
                     Field::make('select', 'quan_huyen_cuoi', __('Quận / Huyện (điểm cuối)', 'mooms'))
                          ->add_options(['---- Chọn quận / huyện ----'])
                          ->set_width(33),
                     Field::make('select', 'phuong_xa_cuoi', __('Phường / Xã (điểm cuối)', 'mooms'))
                          ->add_options(['---- Chọn phường / xã ----'])
                          ->set_width(34),
                     Field::make('text', 'toa_do_x_cuoi', __('Tọa độ X (điểm cuối)', 'mooms'))
                          ->set_width(50),
                     Field::make('text', 'toa_do_y_cuoi', __('Tọa độ Y (điểm cuối)', 'mooms'))
                          ->set_width(50),
                 ]);
    ...
    {
        Container::make('post_meta', __('Thông tin vị trí', 'mooms'))
                 ->set_context('carbon_fields_after_title')// normal, advanced, side or carbon_fields_after_title
                 ->set_priority('default')// high, core, default or low
                 ->where('post_type', 'IN', [$this->post_type])
                 ->add_fields([
                     Field::make('text', 'dia_chi', __('Địa chỉ', 'mooms'))
                          ->set_width(100),
                     Field::make('select', 'tinh', __('Tỉnh / Thành phố', 'mooms'))
                          ->add_options(function () {
                              $posts   = get_posts([
                                  'post_type'      => 'dia-phan-tinh',
                                  'posts_per_page' => -1,
                          ...
                          ->set_width(33),
                     Field::make('select', 'quan_huyen', __('Quận / Huyện', 'mooms'))
                          ->add_options(['---- Chọn quận / huyện ----'])
                          ->set_width(33),
                     Field::make('select', 'phuong_xa', __('Phường / Xã', 'mooms'))
                          ->add_options(['---- Chọn phường / xã ----'])
                          ->set_width(34),
                     Field::make('text', 'toa_do_x', __('Tọa độ X', 'mooms'))
                          ->set_width(50),
                     Field::make('text', 'toa_do_y', __('Tọa độ Y', 'mooms'))
                          ->set_width(50),
                 ]);
